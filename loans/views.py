...
@csrf_exempt
def loanapp(request):                                                                                               # Consumes the loan application json 
    html = "<html><body>Thank You! Application Successful!</body></html>"                                           # Displays Confirmation Message
    html1 = "<html><body>Thank You! Application Successful! Duplicate Exists , Record Updated</body></html>"
    files = request.FILES
    raw_data = files.get('file').read()
    parsed = json.loads(raw_data)
    id1 = int(parsed['CFApplicationData']['FilterID'])
    if(Requests.objects.filter(filter_id=id1).exists()):
        add_to_model(parsed)
...
@csrf_exempt            
def status(request):                                                                                                # Checks Status for Loan Application, given the id        
    html = "<html><body>Sorry! Record not Found </body></html>"                                             
    id = request.GET.get('id')
    if(Requests.objects.filter(filter_id=id).exists()):
        r = Requests.objects.get(filter_id=id)
        b = r.business
        response = JsonResponse({'Status':'Applied','Name':b.name,'LoanId':id,'RequestedLoanAmount':r.loan_amount,'Contactinfo':b.phone,'ApplicationUpdated':b.has_been_updated})
        return response
    else:                                                                                                          
        return HttpResponse(html)                                                                                 # Returns Record not Found if status for non existent application is requested 

def add_to_model(parsed):                                                                                         # Algorithm to update existing record, or create new record
    id2 = int(parsed['CFApplicationData']['FilterID'])
    if(Requests.objects.filter(filter_id=id2).exists()):
